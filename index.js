...
const createProgressTracker = (_patientId) => {
	const steps = [
	...
	const updateDisplay = () => {
		...
		process.stdout.write(
			`\r${step.emoji} ${step.text}${dots} [${currentStep + 1}/${steps.length}]`,
		);
		loadingDots++;
	};
	...
	return {
		...
		complete: (success) => {
			...
			if (success) {
				...
				process.stdout.write(
					`\r❌ Process failed at step ${currentStep + 1}             \n`,
				);
			}
		},
	};
			...
			for (const folder of folders) {
				...
				try {
					...
					if (!claimIds.success) {
						...
						// Move folder to @Recycle when no claims are found
					...
					if (claimIdArray.length > 0) {
						...
						const claimDetails = await getClaimDetailsAndUpload(
							logginSession.sessionId,
							claimIdArray,
							claimCodes,
							patientId,
						...
						// Move folder to @Recycle when there are no claims to process
						moveFolderToRecycle(process.env.BASE_URL, patientId);
					}
				} catch (error) {
					progress.complete(false);
