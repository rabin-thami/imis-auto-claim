...
const isApiSuccess = (response) => {
    return response && response.data && !response.data.error && response.data.result !== false;
}

export const getClaimDetailsAndUpload = async (sessionId, claimId, claimCodes = null, patientId = null, progressTracker = null) => {
    ...
    for (const id of claimId) {
        try {
            ...
            const readParams = {
                "args": [[id], ["package_claim", "message_follower_ids", "visit_type", "insurance_claim_history", "careType", "claim_id", "message_ids", "attachment_ids", "partner_id", "insurance_claim_line", "state", "hospital_diagnosis", "claim_code", "icd_code", "nhis_number", "claimed_amount_total_package", "sale_orders", "nmc", "claimed_amount_total", "started_date", "ended_date", "ipd_code", "claimed_received_date", "onepager_attachment_ids", "claim_manager_id", "access_code", "rejection_reason", "display_name", "__last_update"]],
                "kwargs": {
                    "context": {
                        "lang": "en_US",
                        "tz": "Asia/Kathmandu",
            ...
            const readResponse = await makeOdooApiCall("/web/dataset/call_kw/insurance.claim/read", sessionId, "read", "insurance.claim", readParams);
            ...
            const confirmResponse = await makeOdooApiCall("/web/dataset/call_button", sessionId, "action_confirm", "insurance.claim", confirmParams);
            ...
            if (id === firstClaimId) {
                ...
                const generateOnepagerParams = {
                    "domain_id": null,
                    "context_id": 1,
                    "args": [[id], {"lang": "en_US", "tz": "Asia/Kathmandu", "uid": 9, "params": {"action": 399}}]
                };

                const generateOnepagerResponse = await makeOdooApiCall("/web/dataset/call_button", sessionId, "print_generated_onepager", "insurance.claim", generateOnepagerParams);
            }
            ...
            const submitClaimResponse = await makeOdooApiCall("/web/dataset/call_button", sessionId, "action_claim_submit", "insurance.claim", submitClaimParams);

            if (submitClaimResponse.data && submitClaimResponse.data.error) {
                ...
                const uploadDocumentParams = {
                    "args": [[id], {"lang": "en_US", "tz": "Asia/Kathmandu", "uid": 9, "params": {"action": 400}}]
                }

                const uploadDocumentResponse = await makeOdooApiCall("/web/dataset/call_button", sessionId, "upload_claim_attachments", "insurance.claim", uploadDocumentParams);
            ...
            console.log(`❌ Error processing claim id ${id}:`, error.message);
